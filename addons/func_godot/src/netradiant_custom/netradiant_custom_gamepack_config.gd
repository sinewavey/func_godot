...
func do_export_file() -> void:
	...
	# .shader
	var shader_text := build_shader_text();
	...
	# game path/scripts/.shader
	# NOTE: To work properly, this should go in the game path. For now, I'm leaving the export to NRC as well, so it can easily
	# be repackaged. However, I believe it shouldn't exist there. We'll need to make a decision for this. - VL
	...
	print("Exporting NetRadiant Custom Default Shader List to ", target_file_path)
	file = FileAccess.open(target_file_path, FileAccess.WRITE)
	if file != null:
		file.store_string(gamepack_name)
		file.close()
